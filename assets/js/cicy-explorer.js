...
function wireUI(section, conn, total) {
  ...
  // Delegated row click + Enter for keyboard nav. One listener for all rows
  // (whose count can reach ~7890 in the unfiltered view).
...
function updateSortIndicators(section, col, dir) {
  ...
  section.querySelectorAll(".cicy-sort-btn").forEach(btn => {
    ...
    if (btn.dataset.col === section.dataset.sortCol) {
      btn.classList.add(section.dataset.sortDir === "asc" ? "is-sort-asc" : "is-sort-desc");
      if (arrow) arrow.textContent = section.dataset.sortDir === "asc" ? " ▲" : " ▼";
    } else if (arrow) {
      arrow.textContent = "";
    }
  });
